    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            for step in range(1, num_steps):
                t_cur = torch.tensor(ts[step - 1], dtype=h.dtype, device=device)
...
def plot_phase_portrait(
    ...
    trajs_2d = [pca.transform(traj) for traj in data["trajectories"]]

    pad = 0.5
    x_min, x_max = h0_2d[:, 0].min() - pad, h0_2d[:, 0].max() + pad
    y_min, y_max = h0_2d[:, 1].min() - pad, h0_2d[:, 1].max() + pad
    ...
    grid_tensor = torch.tensor(grid_64d, dtype=torch.float32).to(
        next(model.parameters()).device
    )
    t_tensor = torch.tensor(t, dtype=torch.float32).to(grid_tensor.device)

    model.eval()
    with torch.no_grad():
        dh_64d = model.odefunc(t_tensor, grid_tensor).numpy()  

    dh_2d = dh_64d @ pca.components_.T   # [G, 2]  — correct linear projection
...
def main():
    ...
    if args.show_untrained:
        ...
        data_untrained = get_hidden_trajectories(
            untrained, test_loader, device, max_samples=300
        )
        plot_phase_portrait(
