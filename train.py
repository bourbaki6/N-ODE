...
def evaluate(model: nn.Module, loader, criterion: nn.Module, device: torch.device) -> tuple:
...
def train(cfg: dict, model: nn.Module, device: torch.device):
    ...
    for epoch in range(1, t_cfg["epochs"] + 1):
        ...
        if epoch % l_cfg.get("save_every", 5) == 0:
            ckpt_path = (
                f"{l_cfg['checkpoint_dir']}/{l_cfg['run_name']}_epoch{epoch}.pt"
            )
