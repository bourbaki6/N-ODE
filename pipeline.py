...
def train_model(model, run_name, epochs, device):
    train_loader, test_loader = get_dataloaders("mnist", batch_size=128, num_workers=0)
    ...
    for epoch in range(1, epochs + 1):
        ...
        nfe = (
            measure_nfe(model, test_loader, device, num_batches = 3)
...
def plot_training_curves(all_results):
    ...
    plt.tight_layout()
    path = str(FIGS / "training_curves.png")
...
def plot_solver_comparison(ckpt_path, device):
    ...
    model = NeuralODEClassifier(hidden_dim=64, num_classes=10, solver="euler", num_steps=10)
    ...
    for ax, xkey, xlabel, ykey, ylabel, title in [
        ...
        for grp, c, lbl in [
            (euler_r, COLORS["ode_euler"], "Euler"),
            (rk4_r,   COLORS["ode_rk4"],  "RK4"),
        ]:
            ax.plot(
                [r[xkey] for r in grp], [r[ykey] for r in grp],
    ...
    plt.tight_layout()
    path = str(FIGS / "solver_comparison.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    _ok(f"Saved {path}")
...
def plot_phase_portrait(ckpt_path, device):
    ...
    def collect(mdl, n=300):
        ...
        return (
            np.concatenate(h0s)[:n],
            np.concatenate(h1s)[:n],
            np.concatenate(ls)[:n],
        )

    trained = NeuralODEClassifier(hidden_dim=64, num_classes=10, solver="euler", num_steps=10)
    ckpt = torch.load(ckpt_path, map_location=device)
    trained.load_state_dict(ckpt["model_state_dict"])
    trained = trained.to(device)

    untrained = NeuralODEClassifier(
        hidden_dim=64, num_classes=10, solver="euler", num_steps=10
    ).to(device)
    ...
    fig.suptitle("Phase Portrait — h(0) -> h(1) trajectories in PCA space",
                 fontsize = 12, fontweight = "bold")
    plt.tight_layout()
    path = str(FIGS / "phase_portrait.png")
    plt.savefig(path, dpi = 150, bbox_inches = "tight")
    plt.close()
    _ok(f"Saved {path}")
...
def plot_summary(all_results):
    ...
    params = [all_results[k]["params"] / 1_000  for k in keys]
    ...
    if "ode_rk4" in all_results and "resnet6" in all_results:
        ...
        axes[1].annotate(
            ...
            bbox=dict(boxstyle="round", facecolor="#EEF2F7", edgecolor="#AAAAAA", alpha=0.9),
        )

    fig.suptitle("Summary — Neural ODE vs ResNet Baseline",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
    path = str(FIGS / "summary.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    _ok(f"Saved {path}")
...
def main():
    ...
    plt.suptitle("MNIST samples (de-normalised)", fontsize=10)
    plt.tight_layout()
    grid_path = str(OUT / "data_grid.png")
    plt.savefig(grid_path, dpi=120, bbox_inches="tight")
    plt.close()
    _ok(f"Saved {grid_path}")

    all_results = {}

    for solver in solvers:
        _hdr(f"Neural ODE  solver={solver}  epochs={args.epochs}")
        model = NeuralODEClassifier(
            hidden_dim=64, num_classes=10, solver=solver, num_steps=10
        )
        rname = f"ode_{solver}"
        ...
        r["solver"] = solver
        all_results[rname] = r
        _ok(f"best_acc={r['best_acc'] * 100:.2f}%  params={r['params']:,}")

    _hdr(f"ResNet-6  epochs={args.epochs}")
